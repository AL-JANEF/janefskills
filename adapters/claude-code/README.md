...
1. **Plugin / marketplace** — `.claude-plugin/plugin.json` lists every capability directory under `skills/`; `.claude-plugin/marketplace.json` makes the repository its own one-plugin marketplace. Namespace is `janef-forge`, so `/janef-forge:forge` (and the deprecated `/janef-forge:janef`) work.
